    ...
    <MainLayout title={service.title} >
      <div className={styles.servicePage}>
        ...
        <section className={styles.sectionContent}>
          {service.description && service.description.map((descr) => (
            <p key={descr} className={styles.content_description}>{descr}</p>
          ))}
          {service.content && (
            <h4 className={styles.content_title}>{service.content.title}</h4>
          )}
          {service.content && service.content.subtitles.map((item) => (
            <p key={item} className={styles.content_description}>{item}</p>
          ))}
          {service.map && (
            <Image src={service.map} alt={'карта доставки'} />
          )}
          {service.repairInfo && (
            <div className={styles.repairInfo}>
              {service.repairInfo.map(({title, list}) => (
                <div key={title} className={styles.repairInfo_item}>
                  ...
                  {list.map((listItem) => (
                    <p key={listItem} className={styles.content_description}>{listItem}</p>
                  ))}
                </div>
              ))}
            </div>
